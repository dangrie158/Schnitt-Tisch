...
def spiral(canvas, x, y, inner_diameter, outer_diameter, rounds=4):
    ...
    for round in range(rounds):
        ...
        p.curveTo(x + inner_radius_x + multiplier(0), y + inner_radius_y + multiplier(0), x + inner_radius_x + multiplier(.25), y - inner_radius_y - multiplier(.25), x,  y - inner_radius_y - multiplier(.5))
        p.curveTo(x - inner_radius_x - multiplier(.5), y - inner_radius_y - multiplier(.75), x - inner_radius_x - multiplier(.75), y + inner_radius_y + multiplier(1), x, y + inner_radius_y + multiplier(1))
...
def get_overlay(overlay_size, page_size, overlap, dpi, pages_x, pages_y):
    ...
    for iy in range(pages_y):
        ...
        y_center = mm_to_points((((iy + 1) * (page_size[1] - overlap)) + overlap / 2), dpi)
        can.line(x1, y_center, x2, y_center)

    for ix in range(1, pages_x):
        x_outer = mm_to_points(ix * (page_size[0] - overlap) + overlap / 2, dpi)
        for iy in range(pages_y):
            y_center = mm_to_points(overlay_size[1] - (((iy - 2) * (page_size[1] - overlap)) + overlap / 2) - (overlay_size[1] / 2), dpi)
            spiral(can, x_outer, y_center, inner_diameter=spiral_size[0], outer_diameter=spiral_size[1], rounds=3)
            marker = chr(ord('A') + ix - 1 + (iy * (pages_x - 1)))
            can.drawCentredString(x_outer - font_size / 2, y_center - font_size / 2, marker)
            can.drawCentredString(x_outer + font_size / 2, y_center - font_size / 2, marker)

    for ix in range(pages_x):
        x_center = mm_to_points(ix * (page_size[0] - overlap) + page_size[0] / 2, dpi)
        for iy in range(1, pages_y):
            y_bottom = mm_to_points((((iy) * (page_size[1] - overlap)) + overlap / 2), dpi)
            spiral(can, x_center, y_bottom, inner_diameter=spiral_size[0], outer_diameter=spiral_size[1], rounds=3)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-f', '--page_format', choices=PAGE_SIZES.keys(), type=str, default='A4')
    ...
    with open(args.input, "rb") as input_stream:
        ...
        pages_x, pages_y = (int(input_size[x] // (page_size[x] - args.overlap)) + 1 for x in range(2))

        output_size = page_size[0] * pages_x, page_size[1] * pages_y
        output_page = pdf.PageObject.createBlankPage(width=mm_to_points(output_size[0], args.dpi), height=mm_to_points(output_size[1], args.dpi))

        output_page.mergePage(page)
        output_page.mergePage(get_overlay(output_size, page_size, args.overlap, args.dpi, pages_x, pages_y))
        ...
        for ix in range(pages_x):
            for iy in range(pages_y):
                ...
                x1, y1 = max(0, ix * (page_size[0] - args.overlap)), \
                    input_size[1] - max(0, iy * (page_size[1] - args.overlap))

                x2, y2 = (x1 + page_size[0]), \
                    (y1 - page_size[1])

                output_page.cropBox.lowerLeft = mm_to_native(x1, y1, args.dpi, input_size)
                output_page.cropBox.upperRight = mm_to_native(x2, y2, args.dpi, input_size)
                output_file.addPage(output_page)

                page_num = ix + ((pages_y - iy - 1) * pages_x) + 1
                filename = f'{args.input.stem}_{args.page_format}_page{page_num:03d}.pdf'
